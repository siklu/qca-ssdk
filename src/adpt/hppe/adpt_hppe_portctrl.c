...
extern sw_error_t adpt_hppe_uniphy_mode_set(a_uint32_t dev_id, a_uint32_t index, a_uint32_t mode);
...
_adpt_hppe_instance0_mode_get(a_uint32_t dev_id, a_uint32_t *mode0)
{
	...
	for(port_id = SSDK_PHYSICAL_PORT1; port_id <= SSDK_PHYSICAL_PORT5; port_id++)
	{
		...
		if(port_id != SSDK_PHYSICAL_PORT5 && (port_interface_mode[dev_id][port_id] ==PORT_SGMII_PLUS ||
...
_adpt_hppe_port_phy_config(a_uint32_t dev_id, a_uint32_t index, a_uint32_t mode)
{
	...
	{
		case PORT_WRAPPER_PSGMII:
			/*interface mode changed form psgmii+usxgmii to psgmii+10gbase
			-r+usxgmii, cannot use port 5 to configure malibu*/
...
adpt_hppe_port_interface_mode_apply(a_uint32_t dev_id)
{
	...
	if(rv)
	{
		...
		ssdk_dt_global_set_mac_mode(dev_id, SSDK_UNIPHY_INSTANCE0, mode0_old);
	...
	{
		rv =_adpt_hppe_instance1_mode_get(dev_id, SSDK_PHYSICAL_PORT5, &mode1);
		if(rv)
		{
			/*restore the mode0 mode1 mode2*/
			ssdk_dt_global_set_mac_mode(dev_id, SSDK_UNIPHY_INSTANCE0, mode0_old);
	...
	rv =_adpt_hppe_instance1_mode_get(dev_id, SSDK_PHYSICAL_PORT6, &mode2);
	...
	rv = adpt_hppe_uniphy_mode_set(dev_id, SSDK_UNIPHY_INSTANCE1, mode1);
	if(rv)
	{
	...
	rv = adpt_hppe_uniphy_mode_set(dev_id, SSDK_UNIPHY_INSTANCE2, mode2);
	if(rv)
	{
	...
	rv = _adpt_hppe_port_phy_config(dev_id, SSDK_UNIPHY_INSTANCE1, mode1);
	if(rv)
	{
		mutex_unlock(&priv->mac_sw_sync_lock);
		/*restore the mode0 mode1 mode2*/
	...
	rv = _adpt_hppe_port_phy_config(dev_id, SSDK_UNIPHY_INSTANCE2, mode2);
	mutex_unlock(&priv->mac_sw_sync_lock);
	if(rv)
	{
		/*restore the mode0 mode1 mode2*/
...
static sw_error_t
adpt_hppe_port_mac_reset(a_uint32_t dev_id, a_uint32_t port_id)
{
	a_uint32_t uniphy_index = 0, mode = 0;
	sw_error_t rv = 0;
	...
	qca_gcc_port_mac_clock_reset(dev_id, port_id);
	if (mode == PORT_WRAPPER_USXGMII) {
...
adpt_hppe_port_interface_mode_switch(a_uint32_t dev_id, a_uint32_t port_id)
{
	...
	if (((port_id != HPPE_MUX_PORT1) && (port_id != HPPE_MUX_PORT2)) ||
		((port_id == HPPE_MUX_PORT1) && (port_mode_new == PHY_PSGMII_BASET))) {
		return SW_NOT_SUPPORTED;
	}
	...
	if (port_mode_new != port_mode_old) {
		...
		if (port_mode_new == PHY_SGMII_BASET) {
			...
			ssdk_dt_global_set_mac_mode(dev_id,
				uniphy_index, PORT_WRAPPER_SGMII_CHANNEL0);
			qca_hppe_port_mac_type_set(dev_id, port_id, PORT_GMAC_TYPE);
			rv = _adpt_hppe_port_mux_mac_set(dev_id, port_id, PORT_GMAC_TYPE);
			SW_RTN_ON_ERROR(rv);
			...
			ssdk_dt_global_set_mac_mode(dev_id,
				uniphy_index, PORT_WRAPPER_USXGMII);
			qca_hppe_port_mac_type_set(dev_id, port_id, PORT_XGMAC_TYPE);
			rv = _adpt_hppe_port_mux_mac_set(dev_id, port_id, PORT_XGMAC_TYPE);
			SW_RTN_ON_ERROR(rv);
		...
		qca_mac_port_switch_status_init(dev_id, port_id);
		adpt_hppe_port_mac_reset(dev_id, port_id);
	}
	return rv;
}
