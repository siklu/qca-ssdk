...
parse_flow_host(struct switch_val *val)
{
	...
	while (switch_ext_p) {
		...
		if (!strcmp(ext_value_p->option_name, "name")) {
			...
			val_ptr[42] = ext_value_p->option_value;
		}  else {
			rv = -1;
			break;
		}
...
parse_qos(const char *command_name, struct switch_val *val)
{
	...
	} else if (!strcmp(command_name, "Ptpri")) {
